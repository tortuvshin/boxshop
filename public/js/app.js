(function() {
        ...
        return {
            noticeDescription: function(actions) {
                return function(notice) {
                    var filter = {},
                        template = (actions && actions[notice.action_type_id] && actions[notice.action_type_id].notice_template) || [];
    ...
    app.controller('PushNoticesController', ['$scope', '$http', '$timeout', 'Templates', function($scope, $http, $timeout, Templates) {
        var loop, ajax, $t;
        var actions = {};
        ...
        var repeat = function() {
            ...
            check = function(notice) {
                ...
                $scope.push = 0;
                $http.get(
                    '/user/notices/check/' + (id || '') + '?date=' + $scope.date, { ignoreLoadingBar: true }
                ).success(function(data) {
