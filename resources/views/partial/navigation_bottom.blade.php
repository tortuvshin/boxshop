<div class="header-bottom-content style="width:100%;">
<div class="container">
    <div class="row">
        <div class="col-md-12 col-xs-12">
            <ul class="line-menu">
                <li><a class="kk-1" href="{{ route('home') }}">Нүүр</a></li>
                <li><a class="kk-1" href="javascript:void(0);" id="category">Мэдээ мэдээлэл</a></li>
                <li><a class="kk-1" href="{{ route('products') }}">Барааны жагсаалт</a></li>
                <li><a class="kk-1" href="#">Дэлгүүр нээх</a></li>
                <li><a class="kk-1" href="#">Дэлгүүрүүд</a></li>
                <li><a class="kk-1" href="#">Брэндүүд</a></li>
                <li><a class="kk-1" href="#">Монголд үйлдвэрлэв</a></li>
                <li><a class="kk-1" href="#">Асуулт хариулт</a></li>
                <li><a class="kk-1" href="#">Холбоо барих</a></li>
            </ul>
        </div>
    </div> 
</div>
    
</div>
